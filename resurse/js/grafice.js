...
function culoareRandom(){
    randInt = (a=0, b=255) => Math.random()*(b-a)+a; // [0, 1) -> [0, b-a) -> [a, b)
...
function actualizeazaDateFacturi(){
    ...
    .then(function(obFacturi) {
        ...
        for (let factura of obFacturi){
            for(let planta of factura.produse){
                dateGrafic[planta.nume] = dateGrafic[planta.nume]?dateGrafic[planta.nume]+1:1
                if(!culoriDate[planta.nume])
                    culoriDate[planta.nume] = culoareRandom();
            }
        }
